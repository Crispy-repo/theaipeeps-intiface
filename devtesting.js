...
(async function() {
    ...
    async function sendCommandsForDevice(device) {
        ...
        for (let type in actuatorGroups) {
            ...
            // Build speeds array (using length of scalarCmdArray, default 0).
            let speeds = Array(scalarCmdArray.length).fill(0);
            indices.forEach(idx => {
    ...
    // In checkMessages, update mappingConfig entries from chat messages and then send commands.
    ...
    function stopMapping() {
        ...
        uniqueDeviceIndices.forEach(async (deviceIndex) => {
            ...
            if (device) {
                ...
                let speeds = Array(scalarCmdArray.length).fill(0);
                try {
                    if (device.vibrate && typeof device.vibrate === "function") {
                        await device.vibrate(speeds);
                    } else if (device.oscillate && typeof device.oscillate === "function") {
                        await device.oscillate(speeds);
                    } else if (device.rotate && typeof device.rotate === "function") {
                        await device.rotate(speeds);
                    } else if (device.linear && typeof device.linear === "function") {
                        await device.linear(speeds);
                    } else {
                        // Fallback: send a ScalarCmd message.
                        let message = {
                            ScalarCmd: {
                                ...
                                Id: getNextMessageId(),
                                Scalars: speeds.map((speed, index) => ({
                                    Index: index,
                                    Speed: speed
                                }))
                            }
